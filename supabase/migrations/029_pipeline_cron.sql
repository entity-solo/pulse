-- 029: Point the pipeline cron jobs at the Supabase Edge Function
-- /functions/v1/pipeline?job=<name> instead of the Vercel routes (028).
-- Nothing is hardcoded: PROJECT_URL and CRON_SECRET are read from Vault when
-- each job fires, so vault secrets named PROJECT_URL and CRON_SECRET must exist
-- before these jobs run (they are not created here because their values are
-- operator-owned).
...
select cron.schedule(
  ...
  $$
    select net.http_post(
      url := rtrim((select decrypted_secret from vault.decrypted_secrets where name = 'PROJECT_URL'), '/') || '/functions/v1/pipeline?job=ingest',
...
select cron.schedule(
  ...
  $$
    select net.http_post(
      url := rtrim((select decrypted_secret from vault.decrypted_secrets where name = 'PROJECT_URL'), '/') || '/functions/v1/pipeline?job=analyze',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name = 'CRON_SECRET')
      ),
...
select cron.schedule(
  ...
  $$
    select net.http_post(
      url := rtrim((select decrypted_secret from vault.decrypted_secrets where name = 'PROJECT_URL'), '/') || '/functions/v1/pipeline?job=gc',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name = 'CRON_SECRET')
      ),
